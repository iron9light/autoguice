...
trait MyNodePrinter {
  ...
  def printer(tree: Tree): String = {
    val buf = new StringBuilder

    //def traverseList(els: List[Tree], level: Int,
...
//        buf.append(" | " + Flags.flagsToString(sym.flags))
...
//        if (sym hasFlag MONOMORPHIC  ) buf.append(" | MONOMORPHIC")
      ...
      def nodeinfo2(tree: Tree): String =
        nodeinfo(tree)//(if (comma) "," else "") + nodeinfo(tree)
      ...
      tree match {
        ...
        case ValDef(mods, name, tpt, rhs) =>
          println("ValDef(Modifiers("+mods.flags+"), ")// nodeinfo(tree))
          //println("  " + symflags(tree))
          println("  \"" + name + "\",")
          ...
          tree match {
            case p: Product =>
              if (p.productArity != 0) {
                ...
                for { i <- (0 until p.productArity); elem = p.productElement(i) } {
                  def printElem(elem: Any, level: Int, comma: Boolean): Unit = elem match {
                    case t: Tree =>
                      traverse(t, level, comma)
                    case xs: List[_] =>
                      println("List(")
                      var i = 0
                      val n = xs.size
                      for (x <- xs) {
                        printElem(x, level+1, i < n - 1)

                        i += 1
                      }
                      println(if (comma) ")," else ")")
                    case _ =>
                      println(elem.toString+(if (comma) "," else ""))
                  }
                  printElem(elem, level+1, i < p.productArity - 1)
                }
                printcln(")")
